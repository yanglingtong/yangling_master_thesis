...
def file_update(d):
    with open('best_para_0.txt', 'a') as f:
      f.write(str(d) + '\n')
    f.close()
...
def model_selection():
    ...
    for train_idx, test_idx in kf_1.split(ds['train']):
        ...
        for train_idx, val_idx in kf_2.split(train_set):
            ...
            df = analysis.trials_dataframes(metric="accuracy", mode="max")

            # Save the DataFrame to a CSV file
            df.to_csv("trials_results_0.csv", index=False)

            print('best trial is below: ')
            print(best_trial)
            # trainer.train()
            # trainer.save_model(output_dir + 'best_model_redo/bm_1')
