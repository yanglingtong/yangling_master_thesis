...
def plot_confusion_matrix(cm, classes,m,
                          normalize=False,
...
def get_perfomance(m):
    ...
    for s in seeds:
        ...
        for n in range(1,11):
            ...
            model_path =  "../../../models/data_aug/manif/"+ m +"/"+str(s)+"/bm_" + str(n)
            model = RobertaForSequenceClassification.from_pretrained(model_path, problem_type= "single_label_classification", num_labels=8).to("cuda")
    ...
    plot_confusion_matrix(cm, classes = unique_labels(y_list), m = m, title = "NB Classifier on Test Set")
    print("The result is below " )
    print(classification_report(y_list, pre_list,digits=3))


def get_all():
    all_model_path = ["all_0.1_0.9","all_0.3_0.7","all_0.5_0.5"]
    #all_model_path = ["all_1_0"]
